...
Extracts Actor and Context Information
...
Selects Relevant Output Columns
...
Open the Log Analytics workspace that receives Entra ID Audit Logs.
Go to Logs.
Paste the KQL query into the query editor.
Optionally adjust the Time range (e.g. Last 24 hours, Last 7 days).
Select Run.
...
In the Sentinel workspace, select Logs.
Ensure the correct workspace is selected (the one with Entra ID Audit Logs).
Paste the query and run it as above.
